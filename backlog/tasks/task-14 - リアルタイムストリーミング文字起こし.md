...
<!-- SECTION:NOTES:BEGIN -->
finalSummary: リアルタイムストリーミング文字起こし（Hypothesis/Confirmed 2系統）を実装。ConfirmedBoundaryTracker（境界確定・flush 補完）+ WhisperKitEngine の transcribeStream（AsyncThrowingStream<TranscriptionStreamUpdate>）+ LiveTranscriptView 2系統表示。codex レビューで MAJOR 2件修正（flush 部分デコードの pending 末尾取りこぼし・最終 flush の windowClipTime=1s による末尾1秒未満の取りこぼし）。main 統合時に streaming 経路へ言語設定（makeWhisperDecodingOptions）を配線し #72 との不整合を解消。PR #76 マージ済み（2026-07-13）。実機検証: streaming 品質・レイテンシ・メモリ。
<!-- SECTION:NOTES:END -->
