...
- [ ] 声紋照合閾値の日本語音声での最適値（暫定 0.82、実測後調整）— 検証手順整備済み・実測待ち（TASK-27）。`AppSettingsModel.embeddingMatchThreshold` を `SpeakerProfileStore` に配線済み・SettingsView に調整 UI（0.5〜0.95）あり。`EmbeddingSimilarityReport`（`Sources/SokkiKit/Diagnostics/`）で実会話の diarization 結果から話者内/話者間の類似度分布を算出し DEBUG ログへ出力できる。実音声で分布を確認し閾値を決定するのはユーザー実測待ち。**注意（レビュー指摘）**: 本レポートは単一録音内の生 embedding 同士の類似度のみを測る。実際の照合は録音間の平均ベクトル vs 過去プロファイルの EMA ベクトルの比較であるため、録音間の再現性（別日・別マイク環境での同一人物認識）を検証するには複数回録音してログを目視比較する必要がある
